package com.innometrics.cqrs.repository.builder;
 ...
 */
public class DRPCTopologyBuilder {
    String _function;
    List<Component> _components = new ArrayList<>();


    public DRPCTopologyBuilder(String function) {
        _function = function;
    }
    ...
    private StormTopology createTopology(DRPCSpout spout) {
        ...
        for (; i < _components.size(); i++) {
            ...
            Map<String, CoordinatedBolt.SourceArgs> source = new HashMap<>();
            if (i == 1) {
                source.put(boltId(i - 1), CoordinatedBolt.SourceArgs.single());
            } else if (i >= 2) {
                source.put(boltId(i - 1), CoordinatedBolt.SourceArgs.all());
            }
            CoordinatedBolt.IdStreamSpec idSpec = null;
            if (i == _components.size() - 1 && component.bolt instanceof CoordinatedBolt.FinishedCallback) {
                idSpec = CoordinatedBolt.IdStreamSpec.makeDetectSpec(PREPARE_ID, PrepareRequest.ID_STREAM);
            }
        ...
        if (streams.size() != 1) {
            throw new RuntimeException("Must declare exactly one stream from last bolt in DRPCTopology");
        }
        String outputStream = streams.keySet().iterator().next();
        List<String> fields = streams.get(outputStream).get_output_fields();
        if (fields.size() != 2) {
            throw new RuntimeException("Output stream of last component in DRPCTopology must contain exactly two fields. The first should be the request id, and the second should be the result.");
        }
    ...
    private class InputDeclarerImpl extends BaseConfigurationDeclarer<LinearDRPCInputDeclarer> implements LinearDRPCInputDeclarer {
        ...
        public LinearDRPCInputDeclarer fieldsGrouping(final Fields fields) {
            addDeclaration((prevComponent, declarer) -> declarer.fieldsGrouping(prevComponent, fields));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer fieldsGrouping(final String streamId, final Fields fields) {
            addDeclaration((prevComponent, declarer) -> declarer.fieldsGrouping(prevComponent, streamId, fields));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer globalGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.globalGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer globalGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.globalGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer shuffleGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.shuffleGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer shuffleGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.shuffleGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer localOrShuffleGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.localOrShuffleGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer localOrShuffleGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.localOrShuffleGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer noneGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.noneGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer noneGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.noneGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer allGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.allGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer allGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.allGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer directGrouping() {
            addDeclaration((prevComponent, declarer) -> declarer.directGrouping(prevComponent));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer directGrouping(final String streamId) {
            addDeclaration((prevComponent, declarer) -> declarer.directGrouping(prevComponent, streamId));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer customGrouping(final CustomStreamGrouping grouping) {
            addDeclaration((prevComponent, declarer) -> declarer.customGrouping(prevComponent, grouping));
            return this;
        }

        @Override
        public LinearDRPCInputDeclarer customGrouping(final String streamId, final CustomStreamGrouping grouping) {
            addDeclaration((prevComponent, declarer) -> declarer.customGrouping(prevComponent, streamId, grouping));
            return this;
        }
